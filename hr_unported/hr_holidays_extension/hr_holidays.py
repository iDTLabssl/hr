...
class hr_holidays(orm.Model):
    ...
    def _employee_get(self):
        ...
        ids = self.pool.get('hr.employee').search([('user_id', '=', uid)])
        if ids:
            return ids[0]
        return False
    ...
    employee_id = fields.Integer(default = '_employee_get')
    number_of_days_temp = fields.Float(default='_days_get')
    ...
    def _needaction_domain_get(self):
        ...
        users_obj = self.env.get('res.users')
        domain = []
        ...
        elif users_obj.has_group('hr_holidays_extension.group_hr_leave'):
    ...
    def onchange_bynumber(self, no_days, date_from, employee_id,
        ...
        employee = ee_obj.browse(employee_id)
        if holiday_status_id:
            hs_data = status_obj.read(holiday_status_id,
        ...
        if (ex_rd and employee.contract_id and
            ...
            times = sched_detail_obj.scheduled_begin_end_times(employee.id,
        ...
        while ((next_dt.weekday() in rest_days and ex_rd)
                or (holiday_obj.is_public_holiday(next_dt.date() ) and ex_ph)):
    ...
    def onchange_enddate(self,employee_id, date_to, holiday_status_id):
        ...
        while ((return_date.weekday() in rest_days and ex_rd)
               or (holiday_obj.is_public_holiday(return_date.date()) 
               and ex_ph)):
