...
class hr_schedule(orm.Model):
    ...
    @api.model
    def _compute_alerts(self, field_name, args):
...
WHERE (date_start <= %s and %s <= date_end)
  ...
  AND id <> %s""", (shd.date_end, shd.date_start, shd.employee_id.id, shd.id))
    ...
    @api.ochange('date_start')
    def onchange_employee_start_date(
            self, employee_id, date_start):
    ...
    @api.model
    def delete_details(self, sched_id):
    ...
    @api.model
    def add_restdays(
            self, schedule, field_name, rest_days=None):
    ...
    def create_details(self, sched_id):
        ...
        if schedule.template_id:
            ...
            for lv in leave_obj.browse(leave_ids):
    ...
    def create_mass_schedule(self):
        ...
        for dept in self.pool.get('hr.department').browse(dept_id):
    ...
    def unlink(self):
        ...
        for schedule in self.browse():
            ...
            if len(detail_ids) > 0:
                detail_obj.unlink(detail_ids)

            schedule_ids.append(schedule.id)

        return super(hr_schedule, self).unlink(schedule_ids)

    @api.multi
    def _workflow_common(self,signal, next_state):

        wkf = netsvc.LocalService('workflow')
        for sched in self.browse():
            for detail in sched.detail_ids:
                wkf.trg_validate(
                    'hr.schedule.detail', detail.id, signal)
            self.write(
                sched.id, {'state': next_state})
        return True
    ...
    def workflow_lock(self):
        ...
        all_locked = True
        for sched in self.browse():
            if self.details_locked([sched.id]):
                self.write(sched.id, {
                           'state': 'locked'})
            else:
                all_locked = False
    ...
    def workflow_unlock(self):
        ...
        all_locked = True
        for sched in self.browse():
            if not self.details_locked([sched.id]):
                self.write(
                    sched.id, {'state': 'unlocked'})
            else:
                all_locked = False
...
class schedule_detail(orm.Model):
    ...
    @api.model
    def _day_compute(self, field_name, args):
        res = dict.fromkeys(ids, '')
        for obj in self.browse():
  ...
  AND id <> %s""", (dtl.date_end, dtl.date_start, dtl.schedule_id.id, dtl.id))
    ...
    def scheduled_begin_end_times(
        ...
        detail_ids = self.search([
            ...
            order='date_start')
        if len(detail_ids) > 0:
            sched_details = self.browse(detail_ids)
    ...
    def _remove_direct_alerts(self):
        ...
        scheds = []
        sched_keys = []
        for sched_detail in self.browse():
    ...
    def workflow_lock(self):
        ...
        for detail in self.browse():
            self.write([detail.id], {
                       'state': 'locked'})
            wkf.trg_validate(
                'hr.schedule', detail.schedule_id.id, 'signal_lock')
    ...
    def workflow_unlock(self):
        ...
        for detail in self.browse():
            self.write([detail.id], {
                       'state': 'unlocked'})
            wkf.trg_validate(
                'hr.schedule', detail.schedule_id.id, 'signal_unlock')
...
class hr_schedule_alert(orm.Model):
    ...
    @api.model
    def _get_employee_id(self, field_name, arg):
        res = {}
        for alrt in self.browse():
            if alrt.punch_id:
    ...
    def _get_normalized_attendance(
            self, employee_id, utcdt, att_ids):

        att_obj = self.pool.get('hr.attendance')
        attendances = att_obj.browse(att_ids)
    ...
    def compute_alerts_by_employee(
        ...
        # TODO - Someone who cares about DST should fix ths
        #
        data = self.pool.get('res.users').read(['tz'])
        dt = datetime.strptime(strDay + ' 00:00:00', '%Y-%m-%d %H:%M:%S')
        lcldt = timezone(data['tz']).localize(dt, is_dst=False)
        utcdt = lcldt.astimezone(utc)
        utcdtNextDay = utcdt + relativedelta(days=+1)
        ...
        rule_ids = rule_obj.search(
            [('active', '=', True)])
        for rule in rule_obj.browse(rule_ids):
            res = rule_obj.check_rule(
                rule,
                detail_obj.browse(sched_detail_ids),
                atnd_obj.browse(attendance_ids)
            )
...
class hr_schedule_alert_rule(orm.Model):
    ...
    @api.model
    def check_rule(self, rule, sched_details, punches):
...
class hr_attendance(orm.Model):
    ...
    def create(self, vals):
        ...
        attendances = [
            (
                obj.employee_id.id, fields.date.context_today(self)
            )
        ]
        self._recompute_alerts(attendances)
...
class hr_term(orm.Model):
    ...
    def _restore_schedule(self):
        ...
        for term in self.browse():
            ...
            # Re-create affected schedules from scratch
            for sched_id in sched_ids:
                sched_obj.delete_details(sched_id)
                sched_obj.create_details(sched_id)
    ...
    def state_cancel(self):
        ...
        res = super(hr_term, self).state_cancel()
        return res
